...
fn normalize(blocks : &mut Vec<i32>) {
    let nb_blocks = blocks.len();
    let idx_max = (0..nb_blocks).rev().max_by_key(|&i| blocks[i]).unwrap();
...
fn solve(blocks : &Vec<i32>, loop_size: bool) -> i32 {
    let mut blocks = blocks.clone();
    let mut tbl = std::collections::HashMap::new();
    for i in 0.. {
        match tbl.insert(blocks.clone(), i) {
            Some(first_encounter) => return if loop_size {
                i - first_encounter
            } else {
                i
            },
